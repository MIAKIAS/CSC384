...
do
    echo "=====================Test $i====================="
    time timeout -s SIGKILL 5 python3 battle.py ./battle_validate_complete/$i.txt ${i}out.txt
    retVal=$?
    if [ $retVal -ne 0 ]; then
        exit $retVal
    fi
...
rm ./testcases_complete_out/*
for i in $(seq 0 301)
do
    echo "=====================New Testcase $i====================="
    time timeout -s SIGKILL 3m python3 battle.py ./testcases_complete/$i.txt ./testcases_complete_out/${i}out.txt
    retVal=$?
    if [ $retVal -ne 0 ]; then
        exit $retVal
    fi
    python3 ./battle_validate/battle_validate.py ./testcases_complete_out/${i}out.txt ./Output_wwz/${i}out.txt
    retVal=$?
    if [ $retVal -ne 0 ]; then
        exit $retVal
    fi
...
rm ./testcases_complete_except_S_out/*
for i in $(seq 0 301)
do
    echo "=====================New Testcase $i====================="
    time timeout -s SIGKILL 3m python3 battle.py ./testcases_complete_except_S/$i.txt ./testcases_complete_except_S_out/${i}out.txt
    retVal=$?
    if [ $retVal -ne 0 ]; then
        exit $retVal
    fi
    python3 ./battle_validate/battle_validate.py ./testcases_complete_except_S_out/${i}out.txt ./Output_wwz/${i}out.txt
    retVal=$?
    if [ $retVal -ne 0 ]; then
        exit $retVal
    fi

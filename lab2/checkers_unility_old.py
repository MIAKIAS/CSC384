...
def is_game_end(state:list, isRed:bool) -> bool:
    ...
    if successors(state, not isRed).qsize() == 0:
        return True
...
def AlphaBeta(state:list, isRed:bool, alpha:int, beta:int, depth:int) -> tuple[list, int]:
    ...
    while not potential_position.empty():
        ...
        if next_position_string in state_cache and depth+1 >= state_cache[next_position_string][-1]:
            ...
            # Add to cache
            state_cache[next_position_string] = (next_value, depth+1)
        '''=================================================================================='''
